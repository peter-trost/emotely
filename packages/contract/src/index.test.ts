...
describe('ask_question input', () => {
  ...
  it('rejects unknown answer types and empty question text', () => {
    assert.equal(
      askQuestionInput.safeParse({ question_id: 'x', question: 'Hi?', answer_type: 'multi_select' })
        .success,
      false,
    );
    assert.equal(
      askQuestionInput.safeParse({ question_id: 'x', question: '', answer_type: 'rating' }).success,
      false,
    );
  });
});
...
describe('complete_session input', () => {
  it('accepts a non-empty summary and rejects an empty one', () => {
    ...
    assert.equal(completeSessionInput.safeParse({ summary: '' }).success, false);
  });
});
...
describe('record_answer input', () => {
  ...
  it('rejects unknown answer types', () => {
    const unknown = { question_id: 'x', answer_type: 'mood_slider', value: 3 };
...
describe('text_list, emoji, longtext values', () => {
  it('text_list and emoji accept non-empty string lists (legacy Firestore shape)', () => {
    assert.deepEqual(answerValueSchemas.text_list.parse(['my wife', 'myself', 'Flutter']), [
      'my wife',
      'myself',
      'Flutter',
    ]);
  ...
  it('longtext accepts one non-empty string, rejects a list', () => {
    assert.equal(answerValueSchemas.longtext.parse('I feel happy because…'), 'I feel happy because…');
