...
Three waits come after the workflow is green, and none of them fails it:

- **Apple's processing, 10–30 min.** External distribution cannot skip it, so
  the macos-26 job now stays alive for it. This is why the iOS job is slower
  than the build alone.
